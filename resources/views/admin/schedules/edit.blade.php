                            ...
                            <select name="ship_id" id="ship_id" required
                                ...
                                @foreach ($ships as $ship)
                                    <option value="{{ $ship->id }}"
                                        {{ old('ship_id', $schedule->ship_id) == $ship->id ? 'selected' : '' }}>
                        ...
                        <div class="grid grid-cols-2 gap-4 mb-4">
                            <div>
                                ...
                                <select name="origin_port_id" id="origin_port_id" required
                                    ...
                                    @foreach ($ports as $port)
                                        <option value="{{ $port->id }}" {{ old('origin_port_id', $schedule->origin_port_id) == $port->id ? 'selected' : '' }}>
                                            {{ $port->name }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div>
                                ...
                                <select name="destination_port_id" id="destination_port_id" required
                                    ...
                                    @foreach ($ports as $port)
                                        <option value="{{ $port->id }}" {{ old('destination_port_id', $schedule->destination_port_id) == $port->id ? 'selected' : '' }}>
                                            {{ $port->name }}
                                        </option>
                                    @endforeach
                                </select>
                        ...
                        <div class="grid grid-cols-2 gap-4 mb-6">
                            <div>
                                ...
                                <input type="number" name="price" id="price"
                                    value="{{ old('price', $schedule->price) }}" required min="0"
                                    class="w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500">
                                @error('price')
                                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                @enderror
